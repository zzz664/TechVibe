import { CodeXml, BadgeDollarSign, Gamepad2 } from "lucide-react";

export const INSIGHT_CATEGORY = [
  { id: 1, label: "IT/프로그래밍", category: "programming", icon: <CodeXml/> },
  { id: 2, label: "암호화폐", category: "crypto", icon: <BadgeDollarSign/> },
  { id: 3, label: "게임", category: "game", icon: <Gamepad2/> },
];

export const POST_CATEGORY = [
  { id: 1, label: "IT/프로그래밍", category: "programming" },
  { id: 2, label: "암호화폐", category: "crypto" },
  { id: 3, label: "게임", category: "game" },
]